...
C     tempGraz  :: 1: grazing is temperature dependent, 0: turn dependence off
     ...
     &    useNO3,
     &    combNO,
C variable traits of each species (traitbvol, traittopt, traitparopt)
     &    traitbvol,
     &    traittopt,
     &    traitparopt,
     &    isPrey,
...
C     hetTempExp         :: [exp(1/degrees C)]         see :numref:`pkg_darwin_temperature_params`
...
C
C     numut_tr           :: [trait^2]              mutation rate
C     min_tr             :: [trait]                minimum trait value
C     max_tr             :: [trait]                maximum trait value
C     max_vr_tr          :: [trait^2]              maximum trait variance
C
C- always defined
...
C     ksatDOFe           :: [mmol Fe m^-3]  half-saturation of DOFe for bacterial growth
     ...
     &    PARchi,
#endif
C Mutation rate of each species
     &    numut_tr,
C Reference trait values for each species
     &    ref_tr,
C Minimum and maximum trait values, maximum variance
     &    min_tr,
     &    max_tr,
     &    max_vr_tr,
     &    mQyield,
...
C     aptype :: optical type (for absorption/scattering spectra)
     ...
     &    num_cov
      ...
      INTEGER group(nTrac)
      INTEGER plank(nTrac)
      INTEGER igroup(nTrac)
      INTEGER jgroup(nTrac)
      INTEGER kgroup(nTrac)
...
C    &    Topt_bygroup,
     ...
     &    chl2cmin
      ...
      _RL biovol(nTrac)
      _RL qcarbon(nTrac)
      _RL biovol_bygroup(nTrac,ngroup)
      _RL Topt_bygroup(nTrac,ngroup)
      _RL PARopt_bygroup(nTrac,ngroup)
